...
jwork.fileUpload.uploadProgressHandler = function(file, bytesLoaded, bytesTotal) {
	try {
		...
		var attach_box = jQuery("#" + this.elementId).find("#attach-box");
		
		attach_box.find("#progress").show();
		
		var progressContainer = fileAreaTable.find("#progressBar_"+file.id);
		var progressContainer2 = attach_box.find("#progress");
		...
		var progressBarInProgress2 = jQuery(progressContainer2.find("p.percent").get(0));
		progressBarInProgress2.width(percent + "%");
		progressBarInProgress2.text(percent);
...
jwork.fileUpload.uploadSuccessHandler = function(file, serverData) {
	try {
		...
		} else if(data.JsonResultCode == "success") {
        	console.log(data.result);
        	var _mockdata = {
            'attachurl': '${ctx}/user_files/'+data.result[0].uploadPullPath,
            'filemime': data.result[0].fileType,
            'filename': data.result[0].fileName,
            'filesize': data.result[0].fileSize
        	};
...
jwork.fileUpload.prototype.getFileList = function() {
	...
	jQuery.ajax({
		url : this.ctx + "/jfile/readDownloadFiles",
		type: "POST",
...
jwork.fileUpload.prototype.initProperties = function() {
	...
	this.uploadMode = "";
	this.uploadPathKey = "";
	this.htmlUrl = this.ctx+"/jfile/jfileuploadForm";
	...
	this.uploadUrl = this.ctx + "/jfile/processUpload";
	this.maxFileSize = 20;
	this.fileType = "all";
...
jwork.fileUpload.prototype.appendFileRow = function(file, isSaved) {
	...
	jQuery("#" + this.elementId).find("#up_size").html(totalSizeStr);
	
	// 파일 컨테이너 생성
	element.find("#noDataRow").hide();
	element.find("#describe").hide();
	element.find("#list").show();
	var fileAreaTable = jQuery("#" + this.elementId).find("#fileAreaTable");
	var fileAreaList = jQuery("#" + this.elementId).find("#list");	
		
	var imgButton = jQuery("<a id='deleteButton_"+file.id+"'' href=\"javascript:;\" class=\"btnlink\">삭제</a>");
...
/*	var fileAreaTableTr =	jQuery("<tr class='fileItem " + trClassName + "' id='"+file.id+"' ></tr>").append(
								...
								jQuery("<td class='cen' ></td>").append(
									imgButton));*/
	var fileAreaTableTr2 =	jQuery("<li class=\"fileItem\" id='"+file.id+"' ></li>").append(
							jQuery("<dl></dl>").append(
							jQuery("<dt title='" + file.name + "'>" + file.name + "(" + fileSizeStr + ")<strong>[저작권위반의심]</strong></dt>")).append(							
							jQuery("<dd class='imgbtn'></dd>").append(imgButton)));
	
	//this.addHighlightEvent(fileAreaTableTr);
	this.addHighlightEvent(fileAreaTableTr2);
	this.addPreviewEvent(spanFileName);
	
	//fileAreaTable.append(fileAreaTableTr);
	fileAreaList.append(fileAreaTableTr2);
	...
	return jQuery("<img src=" + this.ctx + "/resources/swfupload/images/arrow_bg_no1.png />").bind("click", 
				function() {
					var form = jQuery("<form />").attr("method", "get")
												 .attr("action", ctx + "/jfile/readDownloadFile");
...
jwork.fileUpload.prototype.changePreview = function(fileId, seq) {
	...
	jQuery(img).attr("src", this.ctx + "/jfile/preview?fileId=" + fileId + "&fileSeq=" + seq+ "&beanId="+this.beanId+"&uploadPathKey="+this.uploadPathKey+"&useSecurity="+this.useSecurity+"&uploadMode="+this.uploadMode);
};
...
jwork.fileUpload.prototype.deleteFileItem = function(imageButton) {
	...
	var element = jQuery("#" + this.elementId);
	var fileAreaList = jQuery(imageButton).parents().filter("#list");
	var li = jQuery(imageButton).parents().filter("li").get(0);
	...
	// SwfUpload에서 해당 파일 삭제
		...
		var swfFileId = jQuery(li).attr("id");
	...
	if(fileAreaList.find("li").length <= 0) {	
		fileAreaList.hide();
...
jwork.fileUpload.prototype.checkUploadingOneTime = function() {
	...
	jQuery.ajax({
		type: "GET",
		async: false,
		url: this.ctx+"/jfile/uploadingCheck",
		data: {fileId : this.fileId},
		dataType: "json",
		success: function(result){
			switch(result.JsonResultCode){
...
jwork.fileUpload.prototype.uploadFinished = function() {
	...
	jQuery.ajax({
		type: "POST",
		async: false,
		url: this.ctx+"/jfile/afterProcessUploadCompleted",
		data: {	fileId : this.fileId, beanId : this.beanId, uploadPathKey : this.uploadPathKey, useSecurity : this.useSecurity, uploadMode : this.uploadMode },
		dataType: "json",
		success: function(result){			
...
jwork.fileUpload.prototype.showBtnAllDown = function() {
	...
	btnAllDown.bind("click", function(){
		var form = jQuery("<form />").attr("method", "get")
								.attr("action", ctx + "/jfile/downloadAll");
		form.append( jQuery("<input />").attr("type", "hidden")
								   		.attr("name", "fileId")
								   		.attr("value", fileId));
		form.append( jQuery("<input />").attr("type", "hidden")
