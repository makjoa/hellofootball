package net.hellofootball.jfile.service.template;
...
import net.hellofootball.jfile.service.FileUploadCompletedEventListener;
import net.hellofootball.jfile.service.JFile;
import net.hellofootball.jfile.service.JFileDetails;
import net.hellofootball.jfile.service.strategy.JFileUploadTypeFactory;
import net.hellofootball.jfile.utils.KeyHelper;
...
//		jfile.setUseSecurity(fileVo.getUseSecurity());
	...
	public HashMap<String, Object> getUploadInfo(MultipartFile multipartFile, JFileDetails fileVo) {
		...
		String fileMask = getFileMask(multipartFile.getOriginalFilename(), 0, fileVo.getUploadMode(), 
				JProperties.getString(GlovalVariables.DEFAULT_FILE_UPLOAD_PATH_KEY));
		...
		map.put("fileMask", fileMask);
		map.put("fileName", fileName);
		map.put("fileSize", fileSize);
		map.put("fileType", fileType);
		map.put("uploadDirectoryPath", uploadDirectoryPath);
		map.put("uploadPullPath", uploadPullPath);
		
		
		if(logger.isDebugEnabled()) {
			StringBuilder sb = new StringBuilder();
			sb.append("\n================Upload Completed====================\n");
			sb.append("UPLOAD PullPATH : "+uploadPullPath+"\n");
			sb.append("UPLOAD PATH : "+uploadDirectoryPath+"\n");
			try {
				sb.append("UPLOAD File : "+multipartFile.getInputStream()+"\n");
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}			
			sb.append("UPLOAD File : "+multipartFile.getContentType()+"\n");			
			sb.append("UPLOAD File : "+multipartFile.getOriginalFilename()+"\n");			
			sb.append("UPLOAD fileSize : "+fileSize+"\n");			
			sb.append("UPLOAD fileMask : "+fileMask+"\n");			
